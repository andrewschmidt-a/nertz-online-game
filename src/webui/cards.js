...
var cards = (function() {
  ...
  var all2 = []; //All the cards created.
  var start = 1;
  var end = start + 12;
  ...
  function init(options) {
    ...
    for (let l = 0; l < opt.loop; l++)
      for (var i = start; i <= end; i++) {
        ...
        all2.push(new Card('h', i, opt.table));
        all2.push(new Card('s', i, opt.table));
        all2.push(new Card('d', i, opt.table));
        all2.push(new Card('c', i, opt.table));
      }
    if (opt.blackJoker) {
      all.push(new Card('bj', 0, opt.table));
      all2.push(new Card('bj', 0, opt.table));
    }
    if (opt.redJoker) {
      all2.push(new Card('rj', 0, opt.table));
    }
